...
export default async function EditBNPLPage({ params }: EditBNPLPageProps) {
  ...
  const { data: bnpl } = await supabase
    .from('bnpl')
    .select('*, accounts:accounts(id, name, provider, icon)')
    .eq('id', params.id)
    .eq('user_id', user.id)
    .maybeSingle();
  ...
  const totalAmount = Number(typed.total_amount) || 0;
  const perInstallment = Number(typed.installment_amount) || 0;
  const installmentsPaid = Number(typed.installments_paid) || 0;
  const installmentsTotal = Number(typed.installments_total) || 0;
  const remainingInstallments = Math.max(installmentsTotal - installmentsPaid, 0);
  const remainingBalance = Math.max(totalAmount - perInstallment * installmentsPaid, 0);
      ...
      <GlassCard variant="light" hover={false} className="grid gap-4 sm:grid-cols-4">
        ...
        <div>
          ...
          <p className="text-xs text-text-secondary">
            ...
            {formatCurrency(perInstallment)} each
          </p>
        </div>
        <div>
